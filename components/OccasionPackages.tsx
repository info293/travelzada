...
export default function OccasionPackages({ occasion, title, subtitle }: OccasionPackagesProps) {
        ...
        const fetchPackages = async () => {
            ...
            try {
                ...
                allPackagesSnapshot.forEach((doc) => {
                    ...
                    // Case-insensitive check for occasion
                    const pkgOccasion = (data.Occasion || '').toLowerCase()
                    const targetOccasion = occasion.toLowerCase()

                    if (pkgOccasion.includes(targetOccasion)) {
                        packagesData.push({ id: doc.id, ...data })
                    }
                })

                // Sort by createdAt if avail (placeholder), for now shuffle or sort
                // Randomize for variety if needed, or sort
    ...
    return (
        <section className="py-20 px-4 md:px-8 lg:px-12 relative overflow-hidden bg-gray-50/50">
