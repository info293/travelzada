...
export default function BlogPostPage({ params }: PageProps) {
    ...
    useEffect(() => {
        const fetchBlogPost = async () => {
            if (typeof window === 'undefined' || !db) {
                // Fallback logic for when DB is not available (mostly for ID based routes)
                if (slug.length === 1) {
                    const numericId = parseInt(slug[0])
                    if (!isNaN(numericId)) {
            ...
            try {
                ...
                if (slug.length === 2) {
                    ...
                    const [categoryParam, urlSlug] = slug

                    // 1. Try to find by slug field
                    const qSlug = query(blogsRef, where('slug', '==', urlSlug))
                    ...
                    if (!snapshotSlug.empty) {
                        ...
                        // 2. Fallback: Try to find by title-slug match
                    ...
                    if (!foundPost) {
                        ...
                        if (!isNaN(numericId)) {
                            const localPost = blogPosts.find(p => p.id === numericId)
                            if (localPost) {
                                foundPost = { ...localPost, id: String(localPost.id) }
                                // No docId for local posts
                            }
                        }
                    }
                }
                ...
                if (foundPost) {
                    ...
                    // Increment view count if we have a docId
                    if (docId) {
                        try {
                            ...
                            // Try setting to 1 if increment fails
                            try {
                                const docRef = doc(db, 'blogs', docId!)
    ...
    // Construct canonical URL using the correct category and slug
    const computedCategory = post?.category ? post.category.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '') : 'general'
    const computedSlug = post?.slug || (post?.title ? post.title.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '') : '')
    const computedCanonical = post ? `https://www.travelzada.com/blog/${computedCategory}/${computedSlug}` : ''

    const canonicalUrl = post?.canonicalUrl || computedCanonical || (typeof window !== 'undefined' ? window.location.href : '')
    ...
    const breadcrumbSchema = post ? generateBreadcrumbSchema([
        ...
        { name: post.category || 'Post', url: `https://www.travelzada.com/blog/${slug.join('/')}` },
        { name: post.title },
    ]) : null
                ...
                <section className="pt-24 pb-12 px-4 md:px-8 lg:px-12">
                    <div className="max-w-4xl mx-auto">
                        <div className="flex flex-col gap-12">
                            ...
                            {/* Newsletter Subscription (Centered at Bottom) */}
                                    ...
                                    <div className="mt-16">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                            {relatedPosts.map((relatedPost) => {
                                                ...
                                                const relatedCategory = relatedPost.category ? relatedPost.category.toLowerCase().replace(/\s+/g, '-') : 'general'
                                                return (
                                                    <Link
                                                        key={relatedPost.id}
                                                        href={`/blog/${relatedCategory}/${relatedSlug}`}
                                                        className="block group bg-white border border-gray-200 rounded-xl overflow-hidden hover:shadow-lg transition-shadow"
                                                    >
                                                        {relatedPost.image && (
                                                        ...
                                                        <div className="p-5">
                                                            <h5 className="text-lg font-bold text-gray-900 mb-2 group-hover:text-primary transition-colors line-clamp-2">
                                                                {relatedPost.title}
                                                            </h5>
