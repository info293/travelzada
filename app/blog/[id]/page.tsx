    ...
    const fetchBlogPost = async () => {
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          // Increment view count (using Firestore increment for atomic updates)
          try {
            await updateDoc(docRef, {
              views: increment(1)
            })
          } catch (viewError) {
            // If increment fails (field might not exist), set it to 1
            try {
              await updateDoc(docRef, {
                views: 1
              })
            } catch (setError) {
              console.log('Could not update view count:', setError)
            }
          }
