...
import type { Vendor, VendorLead } from '@/components/admin/types'
...
type TabType = 'packages' | 'blogs' | 'users' | 'destinations' | 'subscribers' | 'contacts' | 'leads' | 'careers' | 'testimonials' | 'dashboard' | 'ai-generator' | 'create-itinerary' | 'customer-records' | 'jobs' | 'authors' | 'vendors' | 'vendor-leads'
...
export default function AdminDashboard() {
  ...
  const fetchVendors = async () => {
    try {
      ...
      let querySnapshot
      try {
        const q = query(collection(dbInstance, 'vendors'), orderBy('createdAt', 'desc'))
        querySnapshot = await getDocs(q)
      } catch {
        querySnapshot = await getDocs(collection(dbInstance, 'vendors'))
      }
  ...
  const fetchVendorLeads = async () => {
    try {
      ...
      let querySnapshot
      try {
        const q = query(collection(dbInstance, 'vendor_leads'), orderBy('createdAt', 'desc'))
        querySnapshot = await getDocs(q)
      } catch {
        querySnapshot = await getDocs(collection(dbInstance, 'vendor_leads'))
      }
  ...
  const handleAddVendor = async (vendorData: Partial<Vendor>) => {
    ...
    await addDoc(collection(dbInstance, 'vendors'), newVendor)
    await fetchVendors()
  }
  ...
  const handleUpdateVendor = async (id: string, vendorData: Partial<Vendor>) => {
    const dbInstance = getDbInstance()
    const now = new Date().toISOString()
    await updateDoc(doc(dbInstance, 'vendors', id), {
      ...vendorData,
      updatedAt: now,
    })
    await fetchVendors()
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-12 py-8">
        ...
        <div className="mb-6">
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-3">
            ...
            {/* Vendor Marketing & Spin Rewards Tab — opens dedicated page */}
            <button
              onClick={() => router.push('/admin/vendor')}
              className={`group relative flex flex-col items-center gap-2 p-4 rounded-xl border-2 transition-all ${activeTab === 'vendors'
                ? 'border-amber-500 bg-amber-50 shadow-md'
                : 'border-gray-200 bg-white hover:border-amber-400 hover:shadow-sm'
                }`}
            >
              <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${activeTab === 'vendors' ? 'bg-gradient-to-br from-amber-500 to-yellow-500 text-white' : 'bg-gray-100 text-gray-600 group-hover:bg-amber-100 group-hover:text-amber-600'
                }`}>
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="text-center">
                <div className={`text-sm font-semibold ${activeTab === 'vendors' ? 'text-amber-600' : 'text-gray-700'}`}>
                  Vendors
                </div>
                <div className="text-xs text-amber-600 font-bold mt-0.5">{vendors.length} Vendors</div>
              </div>
            </button>

            {/* WhatsApp Live Inbox — opens dedicated page */}
            <button
              onClick={() => router.push('/admin/whatsapp-chats')}
              className="group relative flex flex-col items-center gap-2 p-4 rounded-xl border-2 transition-all border-emerald-200 bg-white hover:border-emerald-500 hover:shadow-sm"
            >
              <div className="w-10 h-10 rounded-lg flex items-center justify-center bg-emerald-100 text-emerald-600 group-hover:bg-emerald-600 group-hover:text-white transition-colors">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
                </svg>
              </div>
              <div className="text-center">
                <div className="text-sm font-semibold text-gray-700 group-hover:text-emerald-600 transition-colors">WhatsApp</div>
                <div className="text-xs text-emerald-500 font-bold mt-0.5">↗ Live Inbox</div>
              </div>
            </button>
...
[
  ...
  },
  {
    ...
    "linkText": "Browse Destinations"
                        ...
                        >
                          ...
                          <td className="px-6 py-4">
                            <div className="flex items-center gap-1.5 text-sm font-medium text-gray-900">
                              ...
                              {lead.mobile && (
                                <button
                                  onClick={() => router.push(`/admin/whatsapp-chats?phone=${encodeURIComponent(lead.mobile)}&name=${encodeURIComponent(lead.name)}`)}
                                  className="text-emerald-600 hover:text-emerald-700 bg-emerald-50 hover:bg-emerald-100 p-1.5 rounded-lg transition-colors"
                                  title={`Start WhatsApp Chat with ${lead.name}`}
                                >
                          ...
                          <td className="px-6 py-4">
                            <div className="flex flex-wrap items-center gap-2">
                              {lead.mobile && (
                                <button
                                  onClick={() => router.push(`/admin/whatsapp-chats?phone=${encodeURIComponent(lead.mobile)}&name=${encodeURIComponent(lead.name)}`)}
                                  className="px-3 py-1.5 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 rounded-lg text-sm font-semibold transition-colors flex items-center gap-1.5 border border-emerald-200"
                                >
        ...
        {/* Vendor Marketing & Spin Rewards CRM Tab */}
        {activeTab === 'vendors' && hasPermission('vendors') && (
          <VendorsSection
            vendors={vendors}
